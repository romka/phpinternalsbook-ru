...
Другими словами, `pNext` и `pLast` это указатели на следующий и предыдущий бакеты в пределах одного значения хэша (в примере выше `a` ссылается на `c` и наоборот). `pListNext` и `pListLast` — указатели не следующий и предыдущий бакеты в пределах всего массива, в том порядке, в котором элементы были добавлены в массив (в примере выше, `a` ссылается на `b`, `b` на `c` и наоборот). Таким образом, доступны операции обхода связанного списка в обе стороны как среди элементов с одинаковым хэшем, так и среди всего массива.
...
Вы уже знаете для чего используются указатели `pListHead` и `pListTail` (они указывают на начало и конец двойного связанного списка определяющего порядок элементов в массиве). Указатель `pInternalPointer` нужен для итерирования и указывает на "текущий" бакет.
