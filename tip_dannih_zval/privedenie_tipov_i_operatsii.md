...
```
Эти функции не принимают на вход zval `result`, а вместо этого модифицируют переданный операнд. Учтите, что использование этих функций отличается от выполнения `+ 1` или `- 1` с помощью `add_function()`/`sub_function()`. Например, инкремент `"a"` запишет результат в новую переменную `"b"` (примечание переводчика: если рассмотреть такой php код - `$c = $a++` в переменной `$c` будет значение `$a`, но сама переменная `$a` увеличится на `1`), а вот добавление `"a" + 1` вернет увеличенное на `1` значение `"a"`.
